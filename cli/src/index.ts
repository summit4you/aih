...
async function cmdChat(flags: Record<string, string | boolean>) {
  ...
  // CC-R#3 — prefix-stability bookkeeping (see rebuildRegistry + /usage):
  // the tool set bytes prefix every request; any change invalidates the
  // provider's cached prefix. Track fingerprint + breaks from the start.
  let lastToolsetFp: string | undefined;
  const prefixBreaks: Array<{ ts: number; kind: "toolset" | "system"; note?: string }> = [];
  function rebuildRegistry(): void {
    ...
    // CC-R#3 — the tool set bytes prefix EVERY request; a change (plan↔build
    // switch, extension register/unregister) invalidates the provider's cached
    // prefix and re-reads all cached tokens at full price. Compare against the
    // previous fingerprint, record the break for /usage attribution, and warn
    // once so the operator knows the next request will miss.
    const fp = toolsetFingerprint(registry.schemas());
    if (lastToolsetFp !== undefined && fp !== lastToolsetFp) {
      prefixBreaks.push({
        ts: Date.now(),
        kind: "toolset",
        note: agentMode === "plan" ? "plan mode (write tools hidden)" : "build mode (write tools visible)",
      });
      tuiRef.current?.pushSystem?.(
        `note: tool set changed (${agentMode} mode) — provider prompt cache will miss on the next request`,
      );
    }
    lastToolsetFp = fp;
  }
  ...
  async function handleLine(line: string): Promise<void> {
    ...
    if (input === "/usage") {
      ...
      // CC-R#3: prefix-stability attribution (cache misses outside idle gaps,
      // plus observed tool-set/system prompt breaks from this session).
      const prefixReport = cachePrefixMissAttribution(log.all(), prefixBreaks);
      lines.push(...formatPrefixStability(prefixReport));
      tui.pushSystem(lines.join("\n"));
      return;
    }
    if (input === "/memory") {
