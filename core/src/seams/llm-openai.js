...
import { consumeSSEStream, classifyProviderError, isQuotaExhaustion, QuotaError, ReasoningRunawayError, StallError, } from "./llm-sse.js";
...
const FIRST_TOKEN_TIMEOUT_MS = () => Number(process.env.AIH_FIRST_TOKEN_TIMEOUT_MS ?? "") || 180_000;
const STALL_TIMEOUT_MS = () => Number(process.env.AIH_STALL_TIMEOUT_MS ?? "") || 60_000;
...
const REASONING_ONLY_TIMEOUT_MS = () => Number(process.env.AIH_REASONING_ONLY_TIMEOUT_MS ?? "") || 120_000;
const REASONING_ONLY_MAX_CHARS = () => Number(process.env.AIH_REASONING_ONLY_MAX_CHARS ?? "") || 16_384;
/** Arm a stall watchdog: fires `fire` after `ms` unless disarmed/reset. */
function armStallTimer(ms, fire) {
    if (ms <= 0)
        return { reset() { }, disarm() { } };
...
export class OpenAICompatibleLLM {
     ...
     */
    #notifyDeGrade(cls, reason) {
        const { owner, onCredentialFailure } = this.#options;
        if (!owner || !onCredentialFailure)
            return;
        try {
    ...
    async complete(req) {
        ...
        for (const [k, v] of Object.entries(this.#options.headers ?? {})) {
            ...
            if (out.includes("{sid}"))
                out = out.split("{sid}").join(req.sessionId ?? this.#sid);
            if (out.includes("{rand}"))
                out = out.split("{rand}").join(opencodeIDBody());
            headers[k] = out;
        }
        ...
        for (let attempt = 0; attempt < attempts; attempt += 1) {
            ...
            if (!res.ok) {
                ...
                if (cls === "auth")
                    this.#notifyDeGrade("credential", message);
            ...
            try {
                if (req.onDelta && res.body) {
                    ...
                    const body = new ReadableStream({
                        pull(controller) {
                            return reader.read().then((r) => r.done ? controller.close() : controller.enqueue(r.value));
                        },
                    });
                    let stalled = false;
                    let stallMs = 0;
...
function toResponse(data) {
    ...
    const usage = data.usage ? mapUsage(data.usage) : undefined;
    return {
